...
PC用の縦長の画像ではウッドデッキ特集という文字とセットに関するテキストを配置しています。
...
ウッドデッキのポップなので茶色をメインカラーに使用して、サムネイル画像と同じように親しみやすい雰囲気を目指すために、色付きのアイコンを添えました。`
    },
    {
        id: 8,
        category: category[1],
...
それによってセット内容が誤解されるとトラブルが発生してしまうためセット説明用の画像を作成しました。

この画像は伝わりやすさを重要視して、テキストだけでなく画像による説明も加えました。
...
商品ページ内に載せる画像なので、商品の画像は別で白抜きの画像が載せてあるためこの画像には入れずにポイントだけを伝える役割を持たせました。
...
それらのガラスコレクションケースをキービジュアルに並べることで、その商品ページ以外の商品も見てもらえるようなデザインにしました。
...
17種類のセットがあるので全てのセットのイラストを描くと時間がかかってしまうため、4種類のパーツのイラストを作成し
それらを並べることで全てのセットのレイアウト例を作れるようにしました。
私は高校で製図を学んでいたので、その知識を活かしてイラストは等角図を使用して作成しました。
